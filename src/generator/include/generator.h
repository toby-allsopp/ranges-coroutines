...
/// and `int
/// del_ref()`.
template <class PromiseType> class intrusive_coroutine_handle {
public:
  ...
  intrusive_coroutine_handle &
  operator=(const intrusive_coroutine_handle &other) {
...
template <class ElementType> struct generator<ElementType>::promise_type {
...
void intrusive_coroutine_handle_release(
    std::experimental::coroutine_handle<PromiseType> coro) {
  if (coro && coro.promise().del_ref() == 0)
    coro.destroy();
}
