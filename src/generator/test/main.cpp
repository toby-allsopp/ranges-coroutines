...
template <typename UnaryPredicate> auto filter_co(UnaryPredicate pred) {
...
class filter_view : public ranges::view_adaptor<filter_view<Rng, Fun>, Rng> {
  ...
  public:
    adaptor() = default;
    adaptor(ranges::semiregular_t<Fun> const &fun,
  ...
  adaptor begin_adaptor() const {
    return {fun_, ranges::end(this->mutable_base())};
  }

public:
  filter_view() = default;
  filter_view(Rng &&rng, Fun fun)
      : filter_view::view_adaptor{std::forward<Rng>(rng)},
        fun_(std::move(fun)) {}
};
...
template <class Fun> auto filter_rv(Fun fun) {
